function [database, lenStat] = CalculateSiftDescriptor(rt_img_dir, imgFileExt, rt_data_dir, gridSpacing, patchSize, maxImSize, nrml_threshold)
%==========================================================================
% usage: calculate the sift descriptors given the image directory
%
% inputs
% rt_img_dir    -image database root path
% imgFileExt    -image file extension. E.g.: 'jpg'
...
% maxImSize     -maximum size of the input image; 0 value do not resize.
...
database.path = {}; % contain the sift patches for each image of each class
database.imagePath = {}; % contains the image paths.
database.nclass = 0;
...
for ii = 1:length(subfolders)
    ...
    if ~strcmp(subname, '.') && ~strcmp(subname, '..')
        ...
        frames = dir(fullfile(rt_img_dir, subname, ['*.',imgFileExt]));
        ...
        for jj = 1:c_num
            ...
            if (max(im_h, im_w) > maxImSize) && (maxImSize ~= 0)
            ...
            database.imagePath = [database.imagePath, imgpath];
        end  
    end
end
